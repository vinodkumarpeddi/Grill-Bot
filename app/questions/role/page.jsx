...
const RoleQuestionsPage = () => {
    ...
    const toggleAnswer = useCallback((index) => {
        setExpandedQuestion((prevIndex) => (prevIndex === index ? null : index)); // Toggle answer visibility
    }, []);
            ...
            {loading ? (
                ...
                <ul className="space-y-6">
                    {questions.length > 0 ? (
                            ...
                            >
                                <div className="flex justify-between cursor-pointer" onClick={() => toggleAnswer(index)}>
                                    ...
                                    <HiOutlineArrowNarrowRight
                                        className={`text-xl transform transition-all duration-300 ${expandedQuestion === index ? "rotate-90" : ""}`}
                                    />
                                </div>
                        ...
                        <p className="text-lg text-gray-600">No questions available for this role.</p>
                    )}
                </ul>
            )}
